...
class DashboardManager {
    ...
    init() {
        ...
        console.log(
            "📊 DASHBOARD: Sistema de dashboard inicializado correctamente"
        );
    }
        ...
        this.statsCards.forEach((card, index) => {
            ...
            if (header) {
                header.addEventListener("click", () =>
                    this.toggleCardDetails(card, index)
                );
            }
    ...
    loadCardDetails(card, index) {
        ...
        if (details && !details.dataset.loaded) {
            ...
            setTimeout(() => {
                details.innerHTML = this.getCardDetailsContent(
            ...
            datasets: [
                {
                    label: "Ingresos",
                    data: [
                        2100000, 2300000, 2800000, 2500000, 3200000, 2900000,
                        2700000,
                    ],
                    borderColor: "#00ff66",
    ...
    setupQuickActions() {
        ...
        actionBtns.forEach((btn) => {
            btn.addEventListener(
                "click",
                this.handleQuickAction.bind(this, btn)
            );
        });
    }
    ...
    animateValue(element, newValue) {
        ...
        const animate = (currentTime) => {
            ...
            const value = Math.floor(
                currentValue + (newValue - currentValue) * progress
            );
    ...
    updateNotificationPanel() {
        if (this.notificationPanel) {
            ...
            const count = this.notificationPanel.querySelector(
                ".notification-count"
            );
            if (count) {
                count.textContent = this.notifications.length;
                count.style.display =
                    this.notifications.length > 0 ? "block" : "none";
            }
        }
    }
