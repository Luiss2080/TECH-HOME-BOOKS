...
class SidebarManager {
    ...
    setupNavItem(item) {
        ...
        if (link) {
            link.addEventListener(
                "click",
                this.handleNavClick.bind(this, item)
            );

            // Hover effects for collapsed mode
            link.addEventListener(
                "mouseenter",
                this.showTooltip.bind(this, item)
            );
            link.addEventListener(
                "mouseleave",
                this.hideTooltip.bind(this, item)
            );
        }
        ...
        if (submenu) {
            ...
            if (toggle) {
                toggle.addEventListener(
                    "click",
                    this.toggleSubmenu.bind(this, item)
                );
            }
        }
    }
    ...
    toggleCollapse() {
        ...
        console.log(
            `📱 Sidebar ${this.isCollapsed ? "colapsado" : "expandido"}`
        );
    }
    ...
    showTooltip(item) {
        if (!this.isCollapsed || this.isHovered || window.innerWidth <= 768)
            return;
